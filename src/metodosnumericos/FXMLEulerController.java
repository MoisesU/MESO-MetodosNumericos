...
import metodosnumericos.tools.EulerIteration;
 ...
 * @author MoisésUlises
 */
public class FXMLEulerController implements Initializable {
    ...
    @FXML
    private TableView<EulerIteration> tvTable;
        ...
        if(tfFunc.getText().isEmpty()||tfY.getText().isEmpty()||tfX.getText().isEmpty()||tfH.getText().isEmpty()||tfNumT.getText().isEmpty()){
            ...
            try{
                ...
                if(sci.exec("function [z]=f(x, y), z="+tfFunc.getText()+", endfunction")){
                    sci.exec("tab = euler(f, "+tfX.getText().trim()+","+tfY.getText().trim()+", "+tfH.getText().trim()+", "+(int)Double.parseDouble(tfNumT.getText())+")");
    ...
    private void setTableValues(String[][] table) {
        ObservableList<EulerIteration> values = FXCollections.observableArrayList();
        int j = 0;
        for (int i = 0; i < table.length; i++) {
            values.add(new EulerIteration(table[i]));
        }
        for (String prop: new String[]{"n", "xn", "yn", "fxy", "hfxy", "yn1"}) {
