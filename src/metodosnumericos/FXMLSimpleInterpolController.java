 ...
 * @author MoisésUlises
 */
public class FXMLSimpleInterpolController implements Initializable {
    ...
    private Scilab sci;
    private final File interpolSimple = new File(Config.getFolder()+"interpolacionSimple.sce");
    ...
    public void initialize(URL url, ResourceBundle rb) {
        try {
            ...
            sci.exec(interpolSimple);
        } catch (FileNotFoundException ex) {
            System.err.println("Error: archivo de scripts/interpolacionSimple.sce no encontrado \n"+ex.getMessage());
        }
    ...
    private void calc(ActionEvent event) {
        if(funct.size()<2){
            ...
            try {
                //function y = interpolSimple(table, x)
                sci.exec("f = "+t.getScilabMatrix(f));
                sci.exec("res = interpolSimple(f, "+tfValue.getText()+")");
